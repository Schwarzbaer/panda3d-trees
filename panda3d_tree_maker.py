...
class StemletSheet:
    def __init__(self, tree_root_node, root_node=None, rest_segments=None, root_length=0.0, branch_definition=BoringTree, rng=0, splitting_acc=0.0,
                 split_rotate_acc=0.0, bend_debt=0.0, style=Skeleton, children=None, stem_length=None):
        """
        tree_root_node
...
def stemify(sheet):
    ...
    for split_idx in range(splits + 1):
        ...
        if split_idx == 0:
            ...
            if bd.split_rotate_mode == SplitRotation.HELICAL:
                sheet.split_rotate_acc += bd.split_rotate + gdrng.uniform(-1, 1) * bd.split_rotate_var
            elif bd.split_rotate_mode == SplitRotation.COPLANAR:
                sheet.split_rotate_acc += -bd.split_rotate + gdrng.uniform(-1, 1) * bd.split_rotate_var + 180
            else:
                raise ValueError("Unknown split_rotate_mode {}".format(bd.split_rotate_mode))
        ...
        if bd.segment_type == SegmentType.STEM:
            # Basic taper
            base_diameter = bd.diameter + gdrng.uniform(-1, 1) * bd.diameter_var
            ...
            if bd.taper <= 1:
                ...
                unit_taper = 0
            taper_z = base_diameter * (1 - unit_taper * runlength)  # taper_z_top = 1 - (sheet.rest_segments) / (bd.segments + 1)
            if bd.taper < 1:
                stemlet_diameter = taper_z
            else:
                Z_2 = (1 - runlength) * bd.length
        ...
        # Child stemlets
        if bd.child_definition is not None:
